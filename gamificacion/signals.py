...
def gestionar_insignias_complejas(usuario):
    ...
    # Corregido: Filtramos directamente por el campo 'nivel' del modelo Curso
    cursos_basicos_totales = Curso.objects.filter(nivel='Básico').count() # Añadido para la condición
    cursos_basicos_ids = list(
        Curso.objects.filter(nivel='Básico').values_list('id_curso', flat=True) # Usamos 'id_curso'
    )
    ...
    cursos_basicos_completados_ids = list(
        ProgresoCurso.objects.filter(
            usuario=usuario, completado=True, curso__nivel='Básico' # Usamos 'curso__nivel'
        ).values_list('curso__id_curso', flat=True) # Usamos 'curso__id_curso'
    )

    # Añade los prints de depuración para que veas los resultados
    ...
    # Asegúrate de que haya cursos básicos en el sistema y que los completados coincidan
